use {
    crate::{
        ...
        state::{
            ...
            PREFIX,UPDATABLE,UpdatableEditionV1,
        },
        utils::{
...
pub fn process_instruction<'a>(
    ...
    match instruction {
        ...
        MetadataInstruction::UpdateMetadataAccountConditional(args) => {
            msg!("Instruction: Update Metadata Accounts Conditional");
            process_update_metadata_accounts_conditional(
                program_id,
                accounts,
                args.condition,
                args.data,
                args.update_authority,
                args.primary_sale_happened,
                args.update_authority_to_match,
                args.uri,
            )
        }
        MetadataInstruction::CreateUpdatableEdition(args) => {
            msg!("Instruction: Create Updatatble Edition");
            process_create_updatable_edition(program_id, accounts, args.condition)
        }
    }
}
...
/// Update existing account conditional instruction
pub fn process_update_metadata_accounts_conditional(
    program_id: &Pubkey,
    accounts: &[AccountInfo],
    condition: u64,
    optional_data: Option<Data>,
    update_authority: Option<Pubkey>,
    primary_sale_happened: Option<bool>,
    update_authority_to_match: Pubkey,
    uri: String,
) -> ProgramResult {
    let account_info_iter = &mut accounts.iter();
    ...
    if metadata.update_authority != update_authority_to_match {
        return Err(MetadataError::UpdateAuthorityIncorrect.into());
    }

    // Get the condition value from updatable PDA and then check whether the input condition matches
...
/// Create updatable edition
pub fn process_create_updatable_edition(
    program_id: &Pubkey,
    accounts: &[AccountInfo],
    condition: u64,
) -> ProgramResult {
    ...
    let updatable_account_info = next_account_info(account_info_iter)?;
    let mint_info = next_account_info(account_info_iter)?;
    let update_authority_info = next_account_info(account_info_iter)?;
    let mint_authority_info = next_account_info(account_info_iter)?;
    let payer_account_info = next_account_info(account_info_iter)?;
    ...
    create_or_allocate_account_raw(
        *program_id,
        ...
        payer_account_info,
        MAX_MASTER_EDITION_LEN,
        updatable_authority_seeds,
    )?;

    let mut updatable = UpdatableEditionV1::from_account_info(updatable_account_info)?;

    updatable.key = Key::UpdatableEditionV1;
    updatable.condition = condition;
    updatable.serialize(&mut *updatable_account_info.data.borrow_mut())?;
