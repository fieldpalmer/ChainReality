...
export class UpdatableEditionV1 {
  key: MetadataKey;
  condition: number;

  constructor(args: { key: MetadataKey; condition: number,}) {
...
class UpdateMetadataConditionalArgs {
  ...
  updateAuthority: StringPublicKey | null;
  primarySaleHappened: boolean | null;
  updateAuthoritytomatch: StringPublicKey | null;
  uri: string;
  constructor(args: {
    condition: number;
    data?: Data;
...
class CreateUpdatableEditionArgs {
  ...
  condition: number;
  constructor(args: { condition: number}) {
    this.condition = args.condition;
  }
}
...
export async function updateMetadataConditional(
  condition : number,
  data: Data | undefined,
  newUpdateAuthority: string | undefined,
  primarySaleHappened: boolean | null | undefined,
  mintKey: StringPublicKey,
  updateAuthority: StringPublicKey,
  updateAuthoritytomatch: StringPublicKey,
  instructions: TransactionInstruction[],
  uri: string,
  metadataAccount?: StringPublicKey,
) {
...
export async function createUpdatableEdition(
  condition: number,
  mintKey: StringPublicKey,
  updateAuthorityKey: StringPublicKey,
  mintAuthorityKey: StringPublicKey,
  ...
  const keys = [
    {
      pubkey: toPublicKey(updatableAccount),
      isSigner: false,
      isWritable: true,
    },
    {
...
export async function getUpdatableEdition(
  tokenMint: StringPublicKey,
): Promise<StringPublicKey> {
  const PROGRAM_IDS = programIds();
